...
#ifdef TKO
	int topK;
	std::vector<TOPKCILIST *> topkCIList;
	int curListCounts;

	double optimalUtility;
	double recordSumUtility;

	void RUCStrategy(std::vector<int> &itemset, double eu);
	void Heapify(int cur, int n);
#endif

#ifdef TKO_PE
	void PEStrategy(hash_map<long, double> &evaluation);
#endif

#ifdef TKO_EPB
	void EPBStrategy(std::vector<UtilityList *>& curArr);
#endif
};
